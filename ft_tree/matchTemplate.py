...
def matchTemplatesAndSave(rawlog_path, template_path, break_threshold=0):
    ...
    cur_ID = 0
    f = open(template_path + 'logSequence.txt', 'w')
    out_list = []
    n = 1
...
class Match:
    ...
    def matchLogsFromFile(self, para):
        ...
        out_label_path = para['out_label_path']
        f_seq = open(out_seq_path, 'w')
        f_label = open(out_label_path, 'w')
        short_log = 0
        # short_threshold = 5
        count_zero = 0
        total_num = 0
        data = pd.read_csv(raw_log_path)
        IN = data[["timestamp","level","raw_data"]].values
        for line in IN:
            total_num += 1
            timestamp = str(line[0])
            logtype = line[1].strip()
            if logtype == "INFO":
                label = 0
            else:
                label = 1
            f_label.writelines(str(label) + '\n')
            log_words = ft_tree.getMsgFromNewSyslog((timestamp+" "+line[2]).strip())[1]
            ...
            f_seq.writelines(timestamp + ' ' + str(tag) + '\n')
            if tag == 0:
                count_zero += 1
                # print line
...
if __name__ == "__main__":
    ...
    parser.add_argument('-leaf_num', help='增量学习时的剪枝阈值 ,如果将6改成10，可以看出不同，即LearnTemplateByIntervals会对新来的数据做剪枝', type=int,
                        default=6)
    parser.add_argument('-template_path', help='template_path', type=str, default="../middle/kafka_log.template")
    parser.add_argument('-fre_word_path', help='fre_word_path', type=str, default="../middle/kafka_log.fre")
    parser.add_argument('-log_path', help='log_path', type=str, default='../data/kafka.csv')
    parser.add_argument('-out_seq_path', help='out_seq_path', type=str, default='../data/kafka_log.seq')
    parser.add_argument('-out_label_path', help='out_label_path', type=str, default='../data/kafka_log.label')
    parser.add_argument('-CUTTING_PERCENT', help='增量学习时会用到，正常匹配用不到', type=float, default=0.3)
    parser.add_argument('-NO_CUTTING', help='增量学习时会用到，正常匹配用不到', type=int,
                        default=1)  # 初步设定1时，是前60% 不剪枝 ,全局开关， 当其为0时，全局按照min_threshold剪枝
