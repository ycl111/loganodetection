...
def getLogsAndSave(para):
    ...
    IN = data[["timestamp","raw_data"]].values
    n = 1
    for log in IN:
        n += 1
        log = (str(log[0])+" "+str(log[1])).strip()
        if not log:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data_path', type=str, default='../data/kafka.csv')
    parser.add_argument('-template_path', type=str, default='../middle/kafka_log.template')
    parser.add_argument('-fre_word_path', type=str, default='../middle/kafka_log.fre')
