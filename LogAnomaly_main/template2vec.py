...
class Template2Vec:
    ...
    def vector_to_most_similar(self, in_vector, topn = 1):
        ...
        for t in self.vector_template_tuple:
            ...
            temp_dict[template_index] = self.cos_simarity(in_vector,vector)
        sorted_final_tuple=sorted(temp_dict.items(),key=lambda asd:asd[1] ,reverse=True)
        # print(sorted_final_tuple)
    ...
    def cos_simarity(self, vector1, vector2):
        ''' 计算两个vector的余弦相似度
        Args:
            vector
        Return：
            cos
        '''
        x = cosine_similarity(torch.Tensor(vector1.tolist()),torch.tensor(vector2.tolist()),dim=0)
        return x
        
        
if __name__ == '__main__':
    temp2Vec_file = '../../model/kafka_log.template_vector'
    template_file = '../../middle/kafka_log.template'
